...
class EventHandler(object):
    ...
    def scroll_wheel_event(self, widget, event, *args):
        ...
        if event.direction == gtk.gdk.SCROLL_UP:
            if event.state & gtk.gdk.CONTROL_MASK:
                ...
                self._smart_scroll_up(
                    prefs['number of pixels to scroll per mouse wheel event'])
            else:
                self._scroll_with_flipping(
                    0, -prefs['number of pixels to scroll per mouse wheel event'])
    ...
    def mouse_press_event(self, widget, event):
        ...
        if event.button == 1:
            ...
            self._window.cursor_handler.set_cursor_type(
                constants.NORMAL_CURSOR)
    ...
    def _scroll_down(self):
        """ Scrolls down. """
        self._scroll_with_flipping(
            0, prefs['number of pixels to scroll per key event'])
    ...
    def _scroll_right(self):
        """ Scrolls right. """
        self._scroll_with_flipping(
            prefs['number of pixels to scroll per key event'], 0)
    ...
    def _smart_scrolling(self, small_step, backwards):
        ...
        new_index = self._window.layout.scroll_smartly(
            max_scroll, backwards, swap_axes)
    ...
    def _execute_command(self, cmdindex):
        ...
        commands = [cmd for cmd in manager.get_commands()
                    if not cmd.is_separator()]
